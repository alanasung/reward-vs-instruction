# Design Document: Reward Seeking Versus Instruction Following Transfer

## 1. Purpose

This document is the method contract for **Reward Seeking Versus Instruction Following Transfer**. It specifies what is
...
One-liner: Compare how reward-seeking and instruction-following behaviors transfer across small task shifts.

Hypothesis: Compare how reward-seeking and instruction-following behaviors transfer across small task shifts.
...
|---|---|---|
| Config schema | `rewinst.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `rewinst.configs.loader` | Hydra compose API |
| Device | `rewinst.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `rewinst.models.registry` | Pinned revisions + hardware fit |
| Hooks | `rewinst.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `rewinst.models.generation` | Chat template path recording |
| Data | `rewinst.data` | Synthetic items, splits, manifests |
| Evaluation | `rewinst.evaluation` | Metrics, failure harness, protocols |
| Ablation | `rewinst.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `rewinst.reporting` | Aggregate → MD/TeX/figures |
| Cache | `rewinst.cache` | Versioned artifact store |
...
`src/rewinst/rewinst/` are preserved across regeneration.
